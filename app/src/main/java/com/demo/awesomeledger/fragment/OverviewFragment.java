...
public class OverviewFragment extends Fragment{
    private PieChart OutgoingChart;
    @Nullable
    @Override
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container,
                             @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_overview, container, false);
        OutgoingChart = view.findViewById(R.id.Pie);
        PieData OutgoingData = getOutgoingPieData();
        initOutgingPieChart(OutgoingData);
        return view;
    }
    private void initOutgingPieChart(PieData OutgoingData) {
        OutgoingChart.setHoleRadius(60f);  //半径
        OutgoingChart.setTransparentCircleRadius(25f); // 半透明圈
        OutgoingChart.setHoleRadius(20);  //实心圆

        OutgoingChart.setDrawCenterText(true);  //饼状图中间可以添加文字
        OutgoingChart.setDrawHoleEnabled(true);
        OutgoingChart.setRotationAngle(90); // 初始旋转角度
        OutgoingChart.setUsePercentValues(true);//设置显示百分比
        OutgoingChart.setRotationEnabled(false); // 可以手动旋转
        OutgoingChart.setUsePercentValues(true);  //显示成百分比
        OutgoingChart.setCenterText("支出");  //饼状图中间的文字
        //设置右下角描述
        Description description = new Description();
        description.setText("支出数据");
        description.setTextSize(16);
        description.setPosition(250,100);
        OutgoingChart.setDescription(description);
        //设置数据
        OutgoingChart.setData(OutgoingData);
        //设置下方比例图信息
        Legend mLegend = OutgoingChart.getLegend();  //设置比例图
        ...
        OutgoingChart.animateXY(1000, 1000);  //设置动画
        OutgoingChart.invalidate(); // 重新更新显示
    }
    private PieData getOutgoingPieData() {
